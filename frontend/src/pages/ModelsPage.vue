  ...
  <AppShell>
    ...
    <ModalShell v-model="showForm" :title="editing ? t('models.edit') : t('models.add')" max-width="max-w-3xl" form @submit="save">
      ...
      <div class="grid items-start gap-x-5 gap-y-4 sm:grid-cols-2">
        ...
        <FormField class="sm:col-span-2" :label="t('models.customHeaders')" :hint="t('models.customHeadersHint')" :error="parsedHeaders ? '' : t('models.invalidJson')"><textarea v-model="headersJson" class="focus-ring h-24 w-full rounded-lg border border-[var(--border)] bg-[var(--input-bg)] p-3 font-mono text-xs" spellcheck="false" /></FormField>
      </div>
