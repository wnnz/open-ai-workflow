...
withDefaults(defineProps<{ modelValue: boolean; title: string; description?: string; maxWidth?: string; bodyClass?: string; form?: boolean }>(), { description: '', maxWidth: 'max-w-lg', bodyClass: 'p-5', form: false })
...
<template>
  <Teleport to="body">
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-5" role="presentation" @mousedown.self="close">
      <component :is="form ? 'form' : 'section'" class="surface flex max-h-[92vh] w-full flex-col overflow-hidden rounded-xl shadow-2xl" :class="maxWidth" role="dialog" aria-modal="true" :aria-label="title" @submit.prevent="emit('submit')">
