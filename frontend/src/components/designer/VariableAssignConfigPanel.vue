...
<template>
  <div data-testid="variable-assign-config-panel" class="mt-5 space-y-5">
    <NodeConfigSection :title="t('designer.variableAssignments')" :hint="t('designer.variableAssignmentsHint')">
      ...
      <div class="space-y-3">
        <div v-for="(assignment, index) in config.assignments" :key="index" class="rounded-lg border border-[var(--border)] bg-[var(--panel-subtle)] p-3">
          <div class="grid grid-cols-[minmax(0,1fr)_92px_28px] gap-2">
            <InputText v-model="assignment.name" class="!h-9 font-mono !text-xs" placeholder="variable1" />
            <Select v-model="assignment.type" class="!h-9 !text-xs"><option v-for="type in ['String','Number','Boolean','Object','Array','Any']" :key="type" :value="type">{{ type }}</option></Select>
          ...
          <div class="mt-2 grid grid-cols-[104px_minmax(0,1fr)] gap-2">
            <Select v-model="assignment.operation" class="!h-9 !text-xs"><option value="overwrite">{{ t('designer.assignmentOperations.overwrite') }}</option><option value="append">{{ t('designer.assignmentOperations.append') }}</option><option value="extend">{{ t('designer.assignmentOperations.extend') }}</option><option value="clear">{{ t('designer.assignmentOperations.clear') }}</option></Select>
