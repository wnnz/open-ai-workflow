...
import { ArrowUpFromLine, Check, Copy } from 'lucide-vue-next'
...
<template>
  <NodeConfigSection v-if="variables.length" class="mt-5 border-t border-[var(--border)] pt-5" :title="t('designer.outputVariables')" :hint="t('designer.outputVariablesHint')" :count="variables.length" kind="output" collapsible :default-expanded="false">
    ...
    <div class="overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--panel)]">
      <button v-for="variable in variables" :key="variable.path" type="button" class="grid w-full grid-cols-[minmax(0,1fr)_82px_24px] items-center gap-2 border-b border-[var(--border)] px-3 py-2.5 text-left last:border-0 hover:bg-[var(--panel-subtle)]" :title="t('designer.copyVariable')" @click="emit('copy', variable.path)">
        ...
        <span class="rounded bg-emerald-50 px-1.5 py-1 text-center text-[9px] font-medium text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{{ variable.type }}</span>
