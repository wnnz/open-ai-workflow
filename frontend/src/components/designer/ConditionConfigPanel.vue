...
<template>
  <div class="mt-5">
    <NodeConfigSection :title="t('designer.nodeParameters')" :hint="t('designer.conditionRulesHint')" kind="parameters">
      ...
      <div class="space-y-2">
        <NodeSettingCard v-for="(clause, index) in config.conditions" :key="index">
          ...
          <Select v-model="clause.operator" class="mt-2"><option v-for="operator in operators" :key="operator" :value="operator">{{ t(`designer.conditionOperators.${operator}`) }}</option></Select>
      ...
      <NodeConfigSection v-if="config.expression" class="mt-4 border-t border-[var(--border)] pt-4" :title="t('designer.legacyCondition')" collapsible :default-expanded="false"><VariableField v-model="config.expression" class="font-mono" :groups="variableGroups" multiline :rows="4" :placeholder="t('designer.conditionPlaceholder')" /></NodeConfigSection>
