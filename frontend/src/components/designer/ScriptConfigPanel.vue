...
import { workflowTypeForScriptSchema } from '@/utils/scriptSchema'
...
function cloneSchema(value: any) { return JSON.parse(JSON.stringify(value)) }
...
async function loadRuntime() {
  ...
  try {
    ...
    props.config.input_schema = cloneSchema(runtime.value.input_schema || { type: 'object', properties: {} })
    props.config.output_schema = cloneSchema(runtime.value.output_schema || { type: 'object', properties: {} })
    props.config.inputs ||= {}
  ...
  <div class="mt-5">
    <NodeConfigSection :title="$t('designer.nodeParameters')" :hint="$t('designer.nodeParametersHint')" kind="parameters">
      ...
      <NodeConfigSection v-if="runtime" class="mt-4 border-t border-[var(--border)] pt-4" :title="$t('scripts.outputs')" :hint="$t('scripts.outputSchemaHint')" :count="Object.keys(outputProperties).length" collapsible :default-expanded="false">
        <div class="grid gap-2 sm:grid-cols-2"><NodeSettingCard v-for="(schema, name) in outputProperties" :key="name"><div class="flex items-center gap-2"><Braces :size="13" class="text-[var(--primary)]" /><code class="min-w-0 flex-1 truncate text-xs">{{ name }}</code><span class="muted text-[9px]">{{ workflowTypeForScriptSchema(schema) }}</span></div></NodeSettingCard></div>
      </NodeConfigSection>
    </NodeConfigSection>
    ...
    <NodeConfigSection v-if="runtime" class="mt-5 border-t border-[var(--border)] pt-5" :title="$t('designer.inputVariables')" :hint="$t('scripts.inputSchemaHint')" :count="Object.keys(inputProperties).length" kind="input" collapsible>
      <div class="space-y-2"><NodeSettingCard v-for="(schema, name) in inputProperties" :key="name" :title="String(name)" :type="workflowTypeForScriptSchema(schema)" :required="requiredInputs.has(String(name))" :hint="schema.description"><VariableField v-model="config.inputs[name]" class="font-mono" :groups="variableGroups" :placeholder="$t('designer.selectUpstreamOutput')" /></NodeSettingCard><p v-if="!Object.keys(inputProperties).length" class="muted py-5 text-center text-xs">{{ $t('scripts.noInputs') }}</p></div>
    </NodeConfigSection>
