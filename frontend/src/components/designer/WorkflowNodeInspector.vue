...
<template>
  <aside class="inspector relative z-30 flex w-[400px] max-w-[38vw] shrink-0 flex-col border-l border-[var(--border)] bg-[var(--panel)]">
    <NodeInspectorHeader
      :node="node"
      :node-type="nodeType"
      :running="running"
    ...
    <div v-if="tab === 'settings'" class="min-h-0 flex-1 overflow-y-auto p-4"><slot name="settings" /></div>
