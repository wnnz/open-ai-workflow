...
<template>
  <FormField :label="field.label || field.name" :required="field.required">
    ...
    <Select v-else-if="field.type === 'select'" :model-value="modelValue" :required="field.required" @update:model-value="emit('update:modelValue', $event)"><option value="" :disabled="field.required">{{ field.placeholder || t('designer.selectOption') }}</option><option v-for="option in field.options" :key="option" :value="option">{{ option }}</option></Select>
