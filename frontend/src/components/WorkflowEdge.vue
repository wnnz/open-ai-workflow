...
.edge-action { display: flex; width: 20px; height: 20px; padding: 0; align-items: center; justify-content: center; border: 1px solid var(--border); border-radius: 50%; background: var(--panel); color: var(--muted); line-height: 0; box-shadow: 0 2px 6px rgb(16 24 40 / 12%); transition: opacity .15s ease, border-color .15s ease, color .15s ease, transform .15s ease; }
.edge-action svg { display: block; flex: none; }
...
:global(.workflow-edge-path.connected) { stroke: var(--primary); stroke-width: 2; stroke-dasharray: none; }
