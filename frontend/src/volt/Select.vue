...
<template>
  <div class="relative inline-flex h-10 w-full text-sm" :class="$attrs.class" @focusout="handleFocusOut">
    ...
    <button
      ...
      :disabled="disabled"
      class="focus-ring flex h-full w-full items-center rounded-lg border border-[var(--border)] bg-[var(--panel)] px-3 pr-10 text-left text-[var(--text)] hover:border-[color-mix(in_srgb,var(--primary),var(--border)_65%)] disabled:cursor-not-allowed disabled:opacity-50"
      :class="controlClass"
      @click="toggleMenu"
    ...
    <input
      ...
      :aria-expanded="open"
      :aria-activedescendant="activeIndex >= 0 ? `${menuId}-${activeIndex}` : undefined"
      class="focus-ring h-10 w-full rounded-lg border border-[var(--border)] bg-[var(--panel)] px-3 pr-10 !text-sm text-[var(--text)] placeholder:text-[var(--muted)]"
      :class="controlClass"
