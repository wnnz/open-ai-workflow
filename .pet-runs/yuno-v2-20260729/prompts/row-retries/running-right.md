Create Codex pet row `running-right` for `yuno`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the exact existing Yuno light-pixel identity, pale luminous humanoid silhouette, facial design, proportions, palette, material, and restrained AI companion personality from the approved v1 atlas.. Preserve silhouette, face, palette, material, proportions, markings, and props.
