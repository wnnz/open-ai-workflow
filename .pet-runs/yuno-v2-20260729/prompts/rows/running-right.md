Create one horizontal animation strip for Codex pet `yuno`, state `running-right`.
...
Identity: same pet in every frame: Preserve the exact existing Yuno light-pixel identity, pale luminous humanoid silhouette, facial design, proportions, palette, material, and restrained AI companion personality from the approved v1 atlas.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: Match the existing atlas exactly; crisp light-pixel sprite rendering, no redesign and no new props..
